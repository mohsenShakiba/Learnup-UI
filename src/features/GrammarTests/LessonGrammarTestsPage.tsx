...
export default function LessonGrammarTestsPage () {
  ...
  const tests = useMemo(
    () => (lessonQuery.data?.tests ?? []).filter((test) => test.type === TestType.GRAMMAR),
    [lessonQuery.data],
  );
  ...
  const resetMutation = useMutation({
    mutationFn: (lessonId: number) => TestsService.resetTestResult(lessonId, TestType.GRAMMAR),
    onSuccess: () => {
  ...
  useSectionCompleted(lessonIdNumber, StorySection.GRAMMAR_TEST, isCompleted);
